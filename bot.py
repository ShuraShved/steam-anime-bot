...
MISTRAL_API_KEY = os.environ["AI_KEY"]
client = Mistral(api_key=MISTRAL_API_KEY)

def get_steam_images_and_desc(appid, n_screens=3):
    r = requests.get(
    ...
    d = r[str(appid)]["data"]
    urls = [d["header_image"]] + [
        s["path_full"] for s in d.get("screenshots", [])[:n_screens]
    ]
    return urls, d.get("short_description", "")

def classify_game(appid):
    urls, desc = get_steam_images_and_desc(appid)

    content = [{
        "type": "text",
        "text": (
            "Это обложка и скриншоты игры Steam. Описание: "
            f"{desc}\n\n"
            "Если оно выглядит как китайское или корейское аниме (часто используется в ии генерации) - это не аниме и вердикт должен быть reject железно.\n"
            "Всё сгенерированное ии должно быть reject.\n"
            "Верни ТОЛЬКО JSON без markdown:\n"
            '{"is_anime_art_style": bool, "anime_confidence": 0-100, '
            '"looks_ai_generated_slop": bool, "verdict": "keep"|"reject"|"uncertain"}'
        )
    }]
    for u in urls:
    ...
    resp = client.chat.complete(
        ...
        messages=[{"role": "user", "content": [
            {"type": "text", "text": "Подробно опиши, что изображено на каждой из картинок по порядку. Сделай вывод: это игра в японской аниме-стиле или нет. выбор тебе поможет сделать то, есть ли на картинках персонажи в японской стилистике. если они сгенерированы ии, напиши что нет, это не игра аниме."},
            *[{"type": "image_url", "image_url": u} for u in urls]
        ]}],
    )

    print(resp.choices[0].message.content)

classify_game(4249050)  # пример appid
